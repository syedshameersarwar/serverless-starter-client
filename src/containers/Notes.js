...
const Notes = props => {
  ...
  useEffect(() => {
    const loadNote = () => API.get("notes", `/notes/${props.match.params.id}`);
  ...
  const saveNote = note =>
    API.put("notes", `/notes/${props.match.params.id}`, { body: note });

  const deleteNote = () => API.del("notes", `/notes/${props.match.params.id}`);
